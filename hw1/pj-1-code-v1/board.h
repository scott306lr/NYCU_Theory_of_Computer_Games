...
class board {
   ...
   private:
    ...
    data info4(size_t i, data dat) {
        data old = info4(i);
        info(info() ^ ((old ^ dat) << (4 * i)));
        return old;
    }
   ...
   public:
    ...
    reward place(unsigned pos, cell tile, cell hint_tile) {
        data bak = info();
        if (pos >= 16 || operator()(pos))
            return -1;
        if (hint() == 0 && !extract_hint_from_bag(tile))
            return -1;
        if (hint() != tile)
            return info(bak), -1;
        if (!extract_hint_from_bag(hint_tile))
            return info(bak), -1;
        operator()(pos) = tile;
        last(4);
        return itov(tile);
   ...
   public:
    ...
    friend std::istream& operator>>(std::istream& in, board& b) {
        for (int i = 0; i < 16; i++) {
            while (!std::isdigit(in.peek()) && in.good())
                in.ignore(1);
            in >> b(i);
            b(i) = ttoi(b(i));
        }
